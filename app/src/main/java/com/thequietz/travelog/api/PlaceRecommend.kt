...
interface PlaceRecommend {
    ...
    @GET("/openapi/service/rest/KorService/areaBasedList?ServiceKey=$SERVICE_KEY&MobileOS=AND&MobileApp=Travlelog&_type=json&arrange=P")
    suspend fun requestAreaBased(@Query("areaCode") code: String, @Query("cat1") category: String, @Query("pageNo") pageNo: Int): RecommendResponse
   ...
   /* @GET("/openapi/service/rest/KorService/areaBasedList?ServiceKey=$SERVICE_KEY&MobileOS=AND&MobileApp=Travlelog&cat1=A01&_type=json&arrange=P")
    ...
    suspend fun requestFestival(@Query("eventStartDate") startDate: String, @Query("areaCode") code: String, @Query("pageNo") pageNo: Int): RecommendResponse
